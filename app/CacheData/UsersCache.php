...
class UsersCache
{
	...
	public function profile($id)
	{
		...
		return cache()->remember($cacheKey, Carbon::now()->addSeconds(10), function() use($id) {
			...
			$latestComments = DB::table('comments')->select('idComment as idcomment', 'idReference as idarticle', 'Content as content', 'created_at as create_date', 'updated_at as modify_date')->where('user_likes.idUser', $id)->orderBy('comments.created_at', 'desc')->limit(7)->get();
			$userProfile = [
