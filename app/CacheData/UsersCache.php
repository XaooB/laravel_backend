...
class UsersCache
{
	...
	public function profile($id)
	{
		...
		return cache()->remember($cacheKey, Carbon::now()->addSeconds(10), function() use($id) {
			...
			$latestComments = DB::table('comments')->leftJoin('articles', 'articles.idArticle', '=', 'comments.idArticle')->select('comments.idComment as idcomment', 'comments.idReference as idarticle', 'articles.Title as title', 'comments.Content as content', 'comments.created_at as create_date', 'comments.updated_at as modify_date')->where('comments.idUser', $id)->orderBy('comments.created_at', 'desc')->limit(7)->get();
			$userProfile = [
