...
class UsersCache
{
	...
	public function profile($id)
	{
		...
		return cache()->remember($cacheKey, Carbon::now()->addSeconds(10), function() use($id) {
			...
			$latestComments = DB::table('comments')->leftJoin('articles', 'articles.idComment', '=', 'comments.idComment')->select('idComment as idcomment', 'idReference as idarticle', 'articles.Title as title', 'Content as content', 'created_at as create_date', 'updated_at as modify_date')->where('idUser', $id)->orderBy('comments.created_at', 'desc')->limit(7)->get();
			$userProfile = [
